...
function getFromHeader() {
  let from = process.env.SMTP_FROM || process.env.SMTP_USER || "no-reply@munnarmarathon.com";
  // Sanitize any accidental escape slashes or misplaced quotes from .env
  from = from.replace(/\\"/g, '').replace(/^"/, '').replace(/"$/, '').trim();
  if (!from.includes("<") && process.env.SMTP_USER) {
    return `"Munnar Marathon" <${process.env.SMTP_USER}>`;
  }
  return from;
}
...
export async function sendSignupOTPEmail(toEmail, otpCode, userName = "Runner") {
    ...
    <div style="font-family: Arial, sans-serif; background-color: #f4f6f8; padding: 40px 20px; color: #1e293b;">
      <div style="max-width: 540px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
        <div style="background: linear-gradient(135deg, #064e3b 0%, #047857 100%); padding: 32px 24px; text-align: center; color: #ffffff;">
          <h1 style="margin: 0; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">Munnar Marathon 2027</h1>
          <p style="margin: 6px 0 0 0; font-size: 13px; color: #a7f3d0; text-transform: uppercase; letter-spacing: 2px;">Email Verification</p>
        </div>
        <div style="padding: 32px 28px;">
          <h2 style="margin-top: 0; font-size: 18px; color: #0f172a;">Welcome to the Trail, ${userName}!</h2>
          <p style="font-size: 14px; line-height: 1.6; color: #475569;">
            Thank you for creating your official runner account for Munnar Marathon (<strong>${toEmail}</strong>). Please enter the 6-digit verification code below to activate your account.
          </p>
          <div style="margin: 28px 0; padding: 20px; background: #f0fdf4; border: 1px dashed #059669; border-radius: 12px; text-align: center;">
            <span style="display: block; font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: #047857; margin-bottom: 6px; font-weight: 700;">Your 6-Digit Verification Passcode</span>
            <span style="font-family: monospace; font-size: 36px; font-weight: 800; letter-spacing: 10px; color: #064e3b;">${otpCode}</span>
          </div>
          <p style="font-size: 13px; color: #64748b; line-height: 1.5;">
            ⏱️ This verification code is valid for <strong>5 minutes</strong>. If you did not sign up for a Munnar Marathon account, please disregard this email.
          </p>
        </div>
  ...
  try {
    ...
    console.log(`✅ [SMTP] Signup OTP sent to ${toEmail} (Message ID: ${info.messageId})`);
    ...
    <div style="font-family: Arial, sans-serif; background-color: #f4f6f8; padding: 40px 20px; color: #1e293b;">
      <div style="max-width: 540px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
        ...
        <div style="padding: 32px 28px;">
          ...
          <div style="margin: 28px 0; padding: 20px; background: #f0fdf4; border: 1px dashed #059669; border-radius: 12px; text-align: center;">
            <span style="display: block; font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: #047857; margin-bottom: 6px; font-weight: 700;">Your 6-Digit Reset Passcode</span>
            <span style="font-family: monospace; font-size: 36px; font-weight: 800; letter-spacing: 10px; color: #064e3b;">${otpCode}</span>
          </div>
          <p style="font-size: 13px; color: #64748b; line-height: 1.5;">
            ⏱️ This verification code is valid for <strong>5 minutes</strong>. If you did not request a password reset, please ignore this email or contact support immediately.
          </p>
        </div>
        <div style="background-color: #f8fafc; padding: 16px; text-align: center; font-size: 12px; color: #94a3b8; border-top: 1px solid #f1f5f9;">
          © 2027 Munnar Marathon. Western Ghats, Kerala, India.
